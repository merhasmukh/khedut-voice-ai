...
# ── Persistent HTTP client — reuse TCP+TLS connection across calls ─────────────
# Creating a new AsyncClient every call costs ~1-2s for TLS handshake.
...
def _get_client() -> httpx.AsyncClient:
    ...
    if _http_client is None or _http_client.is_closed:
        _http_client = httpx.AsyncClient(
            timeout=15.0,
            http2=False,               # HTTP/1.1 keeps-alive reliably on Pi
            limits=httpx.Limits(
...
# ── Simple 1-slot embedding cache — avoids re-embedding identical queries ──────
_last_embed_query: Optional[str] = None
_last_embed_vector: Optional[List[float]] = None
...
async def get_embedding(text: str) -> List[float]:
    ...
    Uses a persistent HTTP client (no repeated TLS handshakes) and caches
    the last result so identical back-to-back queries skip the API call.
    """
    global _last_embed_query, _last_embed_vector

    clean_text = text.strip()
    if not clean_text:
        return [0.0] * VECTOR_SIZE

    # Cache hit — same query as last time, return instantly
    if clean_text == _last_embed_query and _last_embed_vector is not None:
        return _last_embed_vector
    ...
    try:
        ...
        if values:
            # Store in cache
            _last_embed_query  = clean_text
            _last_embed_vector = values
            return values
